...
(function (host) {
  ...
  function loadLevel(level, opts = {}) {
    ...
    setStatus("关卡已载入：" + (isBrutalActive() ? "血妈爆难 · 唯一解" : difficultyText(state.difficulty)) + "。", "ok");
  }
  ...
  function handleComplete() {
    ...
    burstConfetti();
    showWin();
    setStatus("通关完成：" + (isBrutalActive() ? "血妈爆难 · 唯一解" : difficultyText(state.difficulty)) + "。", "ok");
  }

  function showWin() {
    if (!els.winPanel) return;
    const difficultyLabel = isBrutalActive() ? "血妈爆难 · 唯一解" : difficultyText(state.difficulty);
    ...
    if (state.mode === "fixed") {
      ...
      els.winNext.textContent = "再虐一张唯一解";
  ...
  function renderRandomControls() {
    ...
    if (state.randomMode === "brutal") {
      els.randomNote.textContent =
        "挑战：超级无敌血妈爆难会生成 4-5 个箱子的紧凑地图，目标数比箱子多——多出来的就是干扰目标。生成后会暴力枚举所有推动方案，确认只有一个解才会上桌；推错一步就死锁。";
    } else if (state.randomMode === "progressive") {
      els.randomNote.textContent =
  ...
  function buildBrutalDecoyCandidate(rng) {
    ...
    const targetPulls = boxCount * (10 + Math.floor(rng() * 6));
    const pullCounts = Array(boxCount).fill(0);
    let lastCrate = -1;
    let lastDir = "";
    ...
    if (totalPulls < boxCount * 3) return null;
    if (pullCounts.some((c) => c < 1)) return null;
  ...
  function buildBrutalLevelName(boxCount, decoyCount) {
    const decoyTag = decoyCount > 0 ? " · " + decoyCount + " 个干扰目标" : "";
    return "超级无敌血妈爆难 · 唯一解 · " + boxCount + " 箱" + decoyTag;
  }
